            ...
            for period in b:
                ...
                if period not in n:
                    ...
                    if b[period][0].block_id is not None:
                        period_text += f"\n<b>Блок:</b> {find(blocks, b[period][0].block_id)}<blockquote expandable>"
                    ...
                    if n[period][0].block_id is not None:
                        period_text += f"\n<b>Блок:</b> {find(blocks, n[period][0].block_id)}<blockquote expandable>"
...
async def update_overrides(info: Types.OverrideUpdateInfo):
    ...
    classrooms = await actions.get_classrooms()
    blocks = await actions.get_blocks()
    events = await actions.get_events()
    ...
    for cl in tracked_classes:
        ...
        if cl in b_classes and cl in n_classes:
            ...
            text = \
                f"<b>Внесены изменения в расписание для класса {class_obj.name}</b>\n\n" + \
                f"<b>День:</b> {info.day}\n"
            ...
            for period in b:
                ...
                if period not in n:
                    ...
                    for entry in b[period]:
                        period_text += f"\n<b>- {find(subjects, entry.subject_id) if entry.subject_id else "*" + find(events, entry.event_id)}</b> ({find(teachers, entry.teacher_id)}, {find(classrooms, entry.classroom_id)})"
                    ...
                    bb: List[Types.OverrideUpdateEntry] = b[period]
                    nn: List[Types.OverrideUpdateEntry] = n[period]
                    
                    exclude: List[Types.OverrideUpdateEntry] = []
                    ...
                    for lesson in bb:
                        ...
                        if lesson.block_id is not None:
                            with_same_blockpart_id = list(filter(lambda a: lesson.block_part_id == a.block_part_id and lesson.block_id == a.block_id, nn))
                            if len(with_same_blockpart_id) > 0:
                                with_same_blockpart_id: Types.OverrideUpdateEntry = with_same_blockpart_id[0]
                                
                                period_text += \
                                    "\n<b>Заменено занятие в блоке:</b>" + \
                                    f"\n<b>Блок:</b> {find(blocks, lesson.block_id)}" + \
                                    (f"\n<b>Предмет:</b> {find(subjects, with_same_blockpart_id.subject_id)}" if with_same_blockpart_id.subject_id else f"\n<b>Мероприятие:</b> {find(events, with_same_blockpart_id.event_id)}") + (f" (вместо {find(subjects, lesson.subject_id) if lesson.subject_id else "*" + find(events, lesson.event_id)})" if lesson.subject_id != with_same_blockpart_id.subject_id or lesson.event_id != with_same_blockpart_id.event_id else "") + \
                                    f"\n<b>Учитель:</b> {find(teachers, with_same_blockpart_id.teacher_id)}" + (f" (вместо {find(teachers, lesson.teacher_id)})" if lesson.teacher_id != with_same_blockpart_id.teacher_id else "") + \
                                    f"\n<b>Кабинет:</b> {find(classrooms, with_same_blockpart_id.classroom_id)}" + (f" (вместо {find(classrooms, lesson.classroom_id)})" if lesson.classroom_id != with_same_blockpart_id.classroom_id else "") + "\n"
                            else:
                                period_text += \
                                    "\n<b>Удалено занятие из блока:</b>" + \
                                    f"\n<b>Блок:</b> {find(blocks, lesson.block_id)}" + \
                                    (f"\n<b>Предмет:</b> {find(subjects, lesson.subject_id)}" if lesson.subject_id else f"\n<b>Мероприятие:</b> {find(events, lesson.event_id)}") + \
                                    f"\n<b>Учитель:</b> {find(teachers, lesson.teacher_id)}" + \
                                    f"\n<b>Кабинет:</b> {find(classrooms, lesson.classroom_id)}\n"
                        else:
